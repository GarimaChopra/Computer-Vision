...
namespace CSImageViewer {
    ...
    public class CSImageViewer : Form {
        ...
        private double Zoom = 1.0; ///< zoom/scale facto
        ...
        private void InitializeComponent ( ) {
            ...
            this.menuItem5.Click += new System.EventHandler(this.CountObjects);
            // 
            // CSImageViewer
            // 
            this.AllowDrop = true;
        ...
        protected override void OnPaint ( PaintEventArgs e ) {
            ...
            if (mMouseMoveValid) {
                ...
                mMouseX = (int)(mMouseX / Zoom);       //to display pixel value as per image 
                ...
                if (mImage == null) {    //when no image is opened (initial  open)
                    ...
                    if (mMouseX < mImage.getW() && mMouseY < mImage.getH()) //to handle Array out of bound for pointer over image boundaries 
                        {
        ...
        //--------------------------------------------------------------
        /** \brief    to  check if  image is binary
         *  \param    sender  sender
         *  \param    e  event args
         */
        ...
        //--------------------------------------------------------------
        /** \brief    to  check if  image border is empty
         *  \param    sender  sender
         *  \param    e  event args
         */
        ...
        //--------------------------------------------------------------
        /** \brief    to  check if  image is 4 connected
         *  \param    sender  sender
         *  \param    e  event args
         */
        private void is4Connected_Check ( object sender, EventArgs e ) {
            ...
            if (mImage != null && mImage.getIsColor() != true) {
                mGImage = (GrayImageData)mImage;
                b1.visit( mGImage );
                if (b1.get() == true) { //binary  image
                    c1.visit( mGImage );
                    if (c1.get() == true)
                        MessageBox.Show( "Image is 4 Connected" + "\nbad count : " + c1.getBadCount() + "\nbad pixel (r,c) : (" + c1.getBadRow() + "," + c1.getBadCol() + ")" );
                    else
        ...
        //--------------------------------------------------------------
        /** \brief    to  zoom  in  and zoom  out image
         *  \param    sender  sender
         *  \param    e  event args
         */
        private void onKeyDown ( object sender, KeyEventArgs e ) {
            if (mImage != null) {
                if (e.KeyValue == 187) {    //zoom  in
                    if (Zoom < 7) {    //code to avoid distortion and overflow error
                        Zoom *= 1.1;
                        this.Invalidate();
                    } else {

                        MessageBox.Show( "Reached Maximum Zoom" );
                    }

                }
                if (e.KeyValue == 189) {   //zoom  out
                    if (Zoom > 0.3) {      //code to avoid distortion and overflow error
                        Zoom *= 0.9;
                        this.Invalidate();
                    } else {

                        MessageBox.Show( "Reached Minimum Zoom" );
                    }
                }
            } else {
                MessageBox.Show( "Load image to zoom" );
            }
        }

        //--------------------------------------------------------------
        /** \brief    to  Count image   objects
         *  \param    sender  sender
         *  \param    e  event args
         */
        private void CountObjects ( object sender, EventArgs e ) {
            ...
            CountObjects  b1=new  CountObjects();
            if (mImage != null && mImage.getIsColor() != true) {
                mGImage = (GrayImageData)mImage; 
                cb.visit( mGImage );
                cb1.visit( mGImage );
                ...
                if(cb.get()==true) {
                    if (max == min) {
                        MessageBox.Show( "Image has no objects, all pixel are same" );
                    } else if (cb1.get() == false) {
                        MessageBox.Show( "Image border must be empty for object counting" );
                    } else if (ck.getBadCount() != 0) {
                        MessageBox.Show( "All region of 1's are not 4-connected" );
                    } else if (ck.get() != true) {
                        MessageBox.Show( "Image is not 4-connected" );
                    }
                    else {
                        ...
                        b1.visit( mGImage );
                        MessageBox.Show( "Image is a Binary image, 4 Connected and has empty borders" + "\nExternal Count : " + b1.getExternal() + "\nInternal Count: " + b1.getInternal() + "\nObject Count : " + b1.get() );
                    }          
            }
            else
                    MessageBox.Show( "Not a binary image" );
            } else
                MessageBox.Show( "Invalid Image" );
        }
    }
